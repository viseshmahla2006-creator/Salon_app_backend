...
// OWNER: apna salon banao ya update karo (photo, services, prices)
...
// OWNER: apna salon dekhna
router.get("/my-salon", protect, ownerOnly, async (req, res) => {
  const salon = await Salon.findOne({ owner: req.user.id });
  res.json(salon);
});

// CUSTOMER: area/city ke hisab se salons dhundo (sirf active subscription wale dikhenge)
...
// Single salon detail (booking page ke liye)
router.get("/:id", async (req, res) => {
  const salon = await Salon.findById(req.params.id).populate("owner", "name phone");
  if (!salon) return res.status(404).json({ message: "Salon nahi mila" });
  res.json(salon);
});

// OWNER: ₹199/month subscription ke liye Razorpay order banao
router.post("/subscribe/create-order", protect, ownerOnly, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Order banane mein error", error: err.message });
  }
});

// OWNER: payment verify karke subscription 30 din ke liye activate karo
router.post("/subscribe/verify", protect, ownerOnly, async (req, res) => {
  try {
    ...
    const salon = await Salon.findOne({ owner: req.user.id });
    if (!salon) return res.status(404).json({ message: "Pehle apna salon banao" });
    ...
    res.json({ message: "Subscription active ho gayi!", salon });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
